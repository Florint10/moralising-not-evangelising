...
\title{Why are Christian \\digital games deemed \\preachy?}  % use \\ here otherwise you get a justified title
                                     % note capitalization of the title (only common 
                                     % words in lower case)
\tagline{Case Studies Into the Formal Elements of \\Christian Entertainment Software and \\How They Affect Player Engagement.}
                                     % tag line
\author{Domenico Agius}              % your full name
\authorID{6500H}                     % your University Identifier
\supervisor{Prof.\ Stefano Gualeni}  % your supervisor(s) name
%\cosupervisor{Dr Who}               % your cosupervisor(s) name - no . in Dr *OPTIONAL* 
                                     % simply comment out the above line if absent
\department{}                        % your department (e.g. Artificial Intelligence)
\faculty{Institute of Digital Games} % your faculty (e.g. ICT)
\degreeName{M.Sc.\ in Digital Games} % the degree you are reading
...
\abnormalparskip{1em}                % Add spacing after each paragraph
...
\mainmatter 
    ...
    \input{chap4/analysis_and_discussion_main}
    \input{chap5/conclusions_main}
    \appendix
        \input{appA/appendix_a_main}     % these are just test names
