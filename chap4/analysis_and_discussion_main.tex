...
\textit{Alum} \parencite{orsie_alum_2006} is a point-and-click adventure game heavily inspired by older titles developed by LucasArts and Sierra Entertainment. Like those games, \textit{Alum} features a scripted narrative which players must progress by interacting with the environment. They can do so through an avatar capable of performing different actions (i.e., ``verbs'') and an inventory, which they can use to store and combine items. In two occasions, the allows the player to diverge from the main sequence of scripted events. However, in both cases, doing so plays a brief narrative sequence, which leads directly to a game over screen.
